...
/**
 * Built on first use, like the database handle it depends on.
 *
 * `drizzleAdapter(db, ...)` reads from the handle as soon as it is
 * constructed, so calling betterAuth() at module scope reconnected the
 * database at import time and put the build back where it started - dying on
 * /_not-found, which reaches this file through the root layout's nav.
 */
function createAuth() {
  return betterAuth({
    baseURL,
